...
//		fluchtchance = 0;
	...
	public void kampfAuswerten() {
		...
		if (ueberlebende.size() > 0 && ueberlebendeGegner <= 0) {
				...
				for (int i = 0; i < soeldner.length; i++) {
					if (soeldner[i] != null) {
						if (soeldner[i].getGesundheitsPunkte() == 0) {
							kampfView.kampfErgebnis
									.appendText(soeldner[i].getName() + " ist tot und hat die Party verlassen.\n");
							soeldner[i] = null;
						}
					}
				}
			...
			kampfView.kampfErgebnis.appendText(
					((int) Math.floor(partyController.getPartyLevel())) + "x " + material.getClass().getSimpleName()
